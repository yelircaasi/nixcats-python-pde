{
  ...
  outputs = {
    ...
    inherit
      (forEachSystem (
        system: {dependencyOverlays = (import ./nix/overlays.nix) ++ [(utils.standardPluginOverlay inputs)];}
      )) dependencyOverlays;
      # these overlays will be wrapped with ${system} and we will call the same utils.eachSystem function later on to access them ================
      
      # === This overlay grabs all the inputs named in the format `plugins-<pluginName>` Once we add this overlay to our nixpkgs, ================
      
      #   we are able to use `pkgs.neovimPlugins`, which is a set of our plugins. add any flake overlays here. ===================================

      # === see :help nixCats.flake.outputs.categories and :help nixCats.flake.outputs.categoryDefinitions.scheme ==============================
    ...
    categoryDefinitions = {
      ...
      # lspsAndRuntimeDeps: this section is for dependencies that should be available at RUN TIME for plugins. Will be available to PATH within neovim terminal this includes LSPs
      lspsAndRuntimeDeps = {
        general = with pkgs; [
          # === essential ========================================================================================================================
          ripgrep
      ...
      # === plugins which are installed, but not loaded, and will be handled by lz.n (not necessarily lazy loaded) ===============================
      optionalPlugins = {
